            ...
            <div class="chooseArea">
              ...
              <dl v-for="(spuSale, index) in spuSaleAttrList" :key="spuSale.id">
                ...
                <dd
                  ...
                  v-for="(attrValue, index) in spuSale.spuSaleAttrValueList"
                  :key="attrValue.id"
                  @click="changeActive(attrValue, spuSale.spuSaleAttrValueList)"
                >
...
export default {
  ...
  methods: {
    changeActive(attrValue, arr) {
      // 遍历，先将所有都置为0，然后再把点击的置为1
      arr.forEach((item) => (item.isChecked = "0"));
      attrValue.isChecked = 1;
    },
    getSkuNum(event) {
      let reg = /\d+/;
      let result = reg.exec(event.target.value);
      this.skuNum = result[0];
    ...
    async addShopCar() {
      try {
        ...
        //      201 接口崩了。。
        // 路由传参的话，如果是对象，刷新就看不到了,我们采用会话存储，但是需要转换成字符串(无法存储对象)
