...
export default {
  ...
  methods: {
    ...
    // 修改产品的个数
    // 传递disNum是因为在服务器那边进行计算，这边的数字是从服务器拿到并渲染的。感觉设计很怪。
    ...
    handler: throttle(async function (type, disNum, cart) {
      switch (type) {
        ...
        case "minus":
          ...
          break;
        case "change":
          disNum = /\d+/.exec(disNum) - cart.skuNum;
          break;
      }
      // 这里要不做一个节流？
      // 需要
    ...
    async deleteCartById(cart) {
      await this.$store.dispatch("deleteCartListById", cart.skuId).then(
        () => {
          this.getData();
        },
        (error) => {
    ...
    updateAllCartChecked: throttle(async function (event) {
      let isChecked = event.target.checked ? "1" : "0";
      await this.$store.dispatch("updateAllCartIsChecked", isChecked).then(
        () => {
          this.getData();
        },
        (error) => {
  ...
  computed: {
    ...
    isAllCheck() {
      return this.cartInfoList.every((item) => item.isCheck == 1);
    },
  },
};
</script>
